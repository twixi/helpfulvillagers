...
public class EntityFishHookCustom extends Entity
{
    ...
    public int shake;
    public EntityFisherman fisherman;
    ...
    public EntityFishHookCustom(World worldIn)
    {
        super(worldIn);
        this.xTile = -1;
        this.yTile = -1;
    ...
    @SideOnly(Side.CLIENT)
    public EntityFishHookCustom(World worldIn, double x, double y, double z, EntityFisherman anglerIn)
    {
        this(worldIn);
        this.setPosition(x, y, z);
        this.ignoreFrustumCheck = true;
        this.fisherman = anglerIn;
    }

    public EntityFishHookCustom(World worldIn, EntityFisherman fishingFisher)
    {
        super(worldIn);
        this.xTile = -1;
        this.yTile = -1;
        this.zTile = -1;
        this.ignoreFrustumCheck = true;
        this.fisherman = fishingFisher;
        
        this.setSize(0.25F, 0.25F);
        this.setLocationAndAngles(fishingFisher.posX, fishingFisher.posY + (double)fishingFisher.getEyeHeight(), fishingFisher.posZ, fishingFisher.rotationYaw, fishingFisher.rotationPitch);
    ...
    public void onUpdate()
    {
        ...
        else
        {
            ...
            {
                ItemStack itemstack = this.fisherman.getCurrentItem();

                if (this.fisherman.isDead || !this.fisherman.isEntityAlive() || itemstack == null || itemstack.getItem() != Items.fishing_rod || this.getDistanceSqToEntity(this.fisherman) > 1024.0D)
                {
                    this.setDead();
                    this.fisherman.fishEntity = null;
                    HelpfulVillagers.network.sendToAll((IMessage)new FishHookPacket(getEntityId(), true, 0, 0, 0));
                    return;
                }
            ...
            for (int i = 0; i < list.size(); ++i)
            {
                ...
                if (entity1.canBeCollidedWith() && (entity1 != this.fisherman || this.ticksInAir >= 5))
                {
                    float f = 0.3F;
            ...
            if (movingobjectposition != null)
            {
                ...
                {
                    if (movingobjectposition.entityHit.attackEntityFrom(DamageSource.causeThrownDamage(this, this.fisherman), 0.0F))
                    {
            ...
            if (!this.inGround)
            {
                ...
                if (!this.worldObj.isRemote && d10 > 0.0D)
                {
                    ...
                    else
                    {
                        ...
                        this.ticksCaughtDelay -= EnchantmentHelper.getLureModifier(this.fisherman) * 20 * 5;
                    }
    ...
    public int handleHookRetraction()
    {
        ...
        else
        {
            ...
            if (this.caughtEntity != null)
            {
                double d0 = this.fisherman.posX - this.posX;
                double d2 = this.fisherman.posY - this.posY;
                double d4 = this.fisherman.posZ - this.posZ;
            ...
            else if (this.ticksCatchable > 0)
            {
                ...
                double d1 = this.fisherman.posX - this.posX;
                double d3 = this.fisherman.posY - this.posY;
                double d5 = this.fisherman.posZ - this.posZ;
                ...
                this.fisherman.worldObj.spawnEntityInWorld(new EntityXPOrb(this.fisherman.worldObj, this.fisherman.posX, this.fisherman.posY + 0.5D, this.fisherman.posZ + 0.5D, this.rand.nextInt(6) + 1));
                i = 1;
            }
    ...
    private ItemStack getFishingResult()
    {
        ...
        int i = EnchantmentHelper.getLuckOfSeaModifier(this.fisherman);
        int j = EnchantmentHelper.getLureModifier(this.fisherman);
    ...
    public void writeSpawnData(ByteBuf buffer) {
    	/* 612 */     buffer.writeInt((this.fisherman != null) ? this.fisherman.getEntityId() : 0);
    	/*     */   }
    public void readSpawnData(ByteBuf additionalData) {
    	/* 617 */     this.fisherman = (EntityFisherman)this.worldObj.getEntityByID(additionalData.readInt());
    	/*     */   }
